...
const netRequest = (params) => {
    return new Promise((resolve, reject) => {
        ...
        req.on('error', ()=>{
            reject({message:"I can't connect to internet. You need internet connection."})
        })
        req.write(postData)
        req.end()
    })
        ...
        if(isPublicFolder){
            ...
            switch(url){
                case '/':
                    ...
                    break
                case '/auth':
                    response.writeHead(301, {"Location": `https://accounts.spotify.com/en/authorize?client_id=${config.ClientID}&response_type=code&redirect_uri=${encodeURIComponent(config.redirect_uri)}&scope=user-read-currently-playing`})
                    response.end()
                    break
                case '/getToken':
                    if(params.code){
